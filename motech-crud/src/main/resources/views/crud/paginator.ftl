    ...
    <div class="paginator" ng-controller="PaginationCtrl">
        <div>
            ...
            <div class="result-count-info pull-left" ng-show="hasResults()">
                <span>Found <span class="bold ng-cloak" ng-cloak>{{data.totalRows}}</span> record(s)</span> |
                <span ng-cloak class="ng-cloak">Displaying: {{firstRowCount}} - {{lastRowCount}} of {{data.totalRows}} row(s)</span>
...
<div class="current-page-info ng-cloak" ng-cloak>{{currentPage}} of {{numberOfPages()}} pages</div>
...
<div class="goto-page">
    Go to
    <input type="text" class="current-page ng-cloak" ng-cloak value="{{currentPage}}"/> page
</div>
</#macro>
